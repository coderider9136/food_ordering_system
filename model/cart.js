...
async function cartView(userData) {
    ...
    let getProduct= await sequelize.query("SELECT dishes.name, dishes.price, dishes.discounted_price, dishes.img_path, cart.quantity FROM product LEFT JOIN cart ON dishes.id = cart.dish_id LEFT JOIN user ON user.id = cart.user_id where user.id = :key", {
